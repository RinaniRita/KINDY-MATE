...
export function Panel({
  ...
  return (
    <section className={`glass-panel rounded-[2rem] border border-white/50 p-6 transition-all duration-300 md:p-8 ${className}`}>
      {(eyebrow || title) ? (
        <div className="mb-6">
          {eyebrow ? <p className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">{eyebrow}</p> : null}
          {title ? <h2 className="mt-1 text-2xl font-black leading-tight tracking-tight text-slate-800 sm:text-3xl">{title}</h2> : null}
        </div>
      ) : null}
      <div className="font-medium text-slate-600">{children}</div>
    </section>
  );
}
...
export function StatusBadge({ state }: { state: "approved" | "pending" | "blocked" | "demo_only" }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-[0.9rem] border px-3 py-1 text-xs font-black shadow-sm ${styles[state]}`}>
      {labels[state]}
    </span>
  );
}
