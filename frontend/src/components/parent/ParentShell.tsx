...
function parentTitle(pathname: string | null) {
  const entry = pageMeta.find((item) => pathname?.startsWith(item.match));
  return entry ?? { title: "Khu phụ huynh", subtitle: "Quản lý phiên dùng app, hồ sơ trẻ và báo cáo an toàn." };
}
...
export function ParentShell({ children }: ParentShellProps) {
  ...
  const accountLabel = session?.user?.username ?? "Tài khoản";
  ...
  const openEntryModal = useCallback(async (childId?: string) => {
    ...
    try {
      ...
      const activeChildId =
        childId ||
        window.localStorage.getItem("active_child_id") ||
        children[0]?.id ||
        "";
      setSelectedChildId(activeChildId);
      await loadEntryContext(children, activeChildId);
    } catch (err) {
      setEntryError(err instanceof Error ? err.message : "Không thể tải dữ liệu để bắt đầu child mode.");
    } finally {
  ...
  async function loadEntryContext(children: ParentChild[], childId: string) {
  ...
  async function handleStartSession() {
    ...
    if (quickRules.entertainmentPaused) {
      setEntryError("Tạm dừng giải trí đang bật. Hãy tắt trước khi bắt đầu phiên.");
      return;
    }
    ...
    try {
      ...
      setEntryError(err instanceof Error ? err.message : "Không thể bắt đầu phiên child mode.");
    } finally {
      setEntryBusy(false);
    }
  }
  ...
  return (
    <main className="theme-parent min-h-screen bg-[#fffdf7] text-slate-800">
      ...
      >
        ...
        <div className="flex-1 px-3 pb-4">
              ...
              return (
                ...
                >
                  <span className="text-lg">
                    {item.href === "/parent/dashboard" ? "◫" : item.href === "/parent/children" ? "◎" : "◌"}
                  </span>
          ...
          <div className="mt-6 grid gap-2">
              <button
                ...
                className="flex min-h-12 items-center gap-3 rounded-[1.25rem] border border-sky-100 bg-sky-50/80 px-4 text-sm font-black text-sky-700 transition hover:bg-sky-100"
              >
              ...
              <button
                ...
                className="flex min-h-12 items-center gap-3 rounded-[1.25rem] border border-emerald-100 bg-emerald-50/90 px-4 text-sm font-black text-emerald-700 transition hover:bg-emerald-100"
              >
              <span className="text-lg">▶</span>
        ...
        <div className="border-t border-slate-100 px-3 py-4">
          <button
            ...
            className="flex w-full items-center gap-3 rounded-[1.25rem] px-3 py-2 text-left hover:bg-slate-100/80"
          >
            <span className={`flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br ${avatarTone(session?.user?.avatar_id)} text-sm font-black text-slate-800 shadow-sm`}>
          ...
          <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-4 sm:px-6 lg:px-8">
            ...
            <div className="hidden items-center gap-2 md:flex lg:hidden">
              ...
              <button
                type="button"
                onClick={() => void openEntryModal()}
                className="rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-2 text-xs font-black text-emerald-700"
              >
                Vào khu trẻ em
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute inset-y-0 left-0 flex w-[88%] max-w-sm flex-col border-r border-white/60 bg-white/95 p-4 shadow-2xl backdrop-blur-xl">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => setDrawerOpen(false)} className="rounded-2xl p-2 text-slate-400">
                ✕
              </button>
            </div>
              ...
              {parentRoutes.map((item) => {
                ...
                return (
                  <Link
                    ...
                    href={item.href}
                    onClick={() => setDrawerOpen(false)}
                    className={`rounded-[1.25rem] px-4 py-3 text-sm font-black ${
            ...
            <div className="mt-6 grid gap-2">
              <button
                ...
                className="rounded-[1.25rem] border border-sky-100 bg-sky-50 px-4 py-3 text-left text-sm font-black text-sky-700"
              >
                Hỏi AI
              </button>
              <button
                type="button"
                onClick={() => void openEntryModal()}
                className="rounded-[1.25rem] border border-emerald-100 bg-emerald-50 px-4 py-3 text-left text-sm font-black text-emerald-700"
              >
                Vào khu trẻ em
              </button>
            </div>
            ...
            <div className="mt-auto rounded-[1.5rem] border border-slate-100 bg-slate-50 p-3">
              <p className="text-sm font-black text-slate-800">{accountLabel}</p>
      ...
      {!isInsightsPage ? (
        <button
          type="button"
          onClick={() => router.push("/parent/insights")}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/45 p-4 backdrop-blur-sm">
          <div className="glass-panel max-h-[88vh] w-full max-w-2xl overflow-y-auto rounded-[2rem] border border-white/70 p-6 md:p-8">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">Bắt đầu child mode</p>
            ...
            {entryLoading ? (
              ...
              <div className="mt-6 grid gap-5">
                ...
                <div className="rounded-[1.5rem] border border-[#dff6ee] bg-[#f3fbf7] p-5">
                  ...
                  <div className="mt-4 flex items-center justify-between rounded-[1.25rem] border border-slate-100 bg-white px-4 py-3">
                    <div>
                      <p className="text-sm font-black text-slate-800">Tạm dừng giải trí</p>
                      <p className="text-xs font-semibold text-slate-500">Nếu đang bật, phiên sẽ không được bắt đầu từ flow này.</p>
                    </div>
                    <button
                      type="button"
                      onClick={() =>
                        setQuickRules((current) => ({
                          ...current,
                          entertainmentPaused: !current.entertainmentPaused,
                        }))
                      }
                      className={`rounded-full px-4 py-2 text-xs font-black ${
                        quickRules.entertainmentPaused ? "bg-rose-500 text-white" : "bg-emerald-100 text-emerald-700"
                      }`}
                    >
                      {quickRules.entertainmentPaused ? "Đang bật" : "Đang tắt"}
                    </button>
                  </div>

                  <p className="mt-4 text-xs font-bold leading-6 text-slate-500">
                    Nghỉ tối thiểu suy ra từ phiên hiện tại:{" "}
                    <strong className="text-slate-700">
                      {Math.max(quickRules.sessionDuration - quickRules.totalScreen, 0)} phút
                    </strong>
                  </p>
                </div>
